...
def create_access_token(
    ...
    encoded: Any = jwt.encode(to_encode, settings.SECRET_KEY, algorithm=settings.ALGORITHM)
    return str(encoded)
...
def decode_access_token(token: str) -> dict[Any, Any] | None:
    ...
    """
    try:
        decoded: Any = jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
        return dict(decoded)
    except JWTError:
        return None
